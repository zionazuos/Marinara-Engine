...
function WidgetBody({ widget }: { widget: HudWidget }) {
  switch (widget.type) {
    ...
    default:
      return <p className={cn("text-[0.625rem]", GAME_WIDGET_MUTED_CLASS)}>Tipo de widget desconhecido</p>;
  }
}
...
function ListWidget({ widget }: { widget: HudWidget }) {
  ...
  return (
    <div className="space-y-0.5">
      {items.length === 0 ? (
        <p className={cn("text-[0.5625rem] italic", GAME_WIDGET_MUTED_CLASS)}>Vazio</p>
      ) : (
